...
export const ImagesCard = ({ images }) => {
  const [imageWidthUnit, setImageWidthUnit] = useState(null);
  ...
  const imageGroups = useMemo(() => {
    return images.reduce((acc, image, idx) => {
      ...
      const groupPartIdx = GROUP_PART_CONFIG.findIndex((idxConfig) => {
        return (
          imageInGroupIndex >= idxConfig.from &&
          imageInGroupIndex <= idxConfig.to
        );
      });
  ...
  return (
    <View style={{ height: "100%" }}>
      <View
        onLayout={(event) =>
          setImageWidthUnit(event.nativeEvent.layout.width / COLUMNS_AMOUNT)
        }
      >
        {imageWidthUnit !== null &&
                ...
                <View style={{ flexDirection: "row" }}>
                  {group[0] &&
                    group[0].map((image, imageIdx) => {
                      const size =
                        imageWidthUnit * GROUP_PART_CONFIG[0].sizeFactor;
                ...
                <View style={{ flexDirection: "row" }}>
                  {group[2] &&
                    group[2].map((image, imageIdx) => {
                      const size =
                        imageWidthUnit * GROUP_PART_CONFIG[2].sizeFactor;
              ...
              <View style={{ flexDirection: "column" }}>
                {group[1] &&
                  group[1].map((image, imageIdx) => {
                    const size =
                      imageWidthUnit * GROUP_PART_CONFIG[1].sizeFactor;
